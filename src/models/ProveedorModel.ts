...
export class ProveedorModel {
  ...
  /**
   * Listar proveedores usando la vista `vista_proveedores`, con filtros parciales y paginación.
   */
  static async findAll(filters: ProveedorFilter) {
    ...
    // filtro por número de documento (DUI, NIT, etc.)
    if (filters.documento) {
      clauses.push(`
        EXISTS (
          SELECT 1
            FROM trn_documento_persona dp
           WHERE dp.id_persona = v.id_persona
             AND dp.numero ILIKE '%' || $${idx++} || '%'
        )
      `);
    ...
    const totRes = await pool.query<{ count: string }>(
      `SELECT COUNT(*) AS count FROM vista_proveedores v ${where};`,
      params
    );
  ...
  /**
   * Crear o vincular persona como proveedor (idempotente)
   */
  static async create(dto: ProveedorDTO): Promise<{ id_persona: string; message: string }> {
    const pool = await this.getPool();
    ...
    try {
      ...
      // Revisar existencia por documentos
      for (const doc of dto.documentos) {
        ...
        if (existDoc.rowCount) {
          ...
          // Vincular categoría Proveedor si falta
          const { rows: catRows } = await client.query<{ id_categoria_persona: string }>(
            `SELECT id_categoria_persona FROM categoria_persona WHERE nombre = 'Proveedor';`
          );
          if (!catRows.length) {
            throw new Error('Categoría Proveedor no configurada');
          }
          const provCatId = catRows[0].id_categoria_persona;

          const existMap = await client.query(
            `SELECT 1 FROM trn_categoria_persona WHERE id_persona = $1 AND id_categoria_persona = $2;`,
            [existingPersonaId, provCatId]
          );
          if (existMap.rowCount === 0) {
            await client.query(
              `INSERT INTO trn_categoria_persona
               ...
               VALUES ($1, $2, CURRENT_DATE, 'PROV', $3);`,
              [existingPersonaId, provCatId, activeStateId]
            );
          }
          ...
          return {
            ...
            message: 'Persona ya existente, mapeo como proveedor realizado.'
          };
        }
      }

      // No existe: crear nombre_persona
      const fullName = [
      ...
      const { rows: persRows } = await client.query<{ id_persona: string }>(
        `INSERT INTO persona
         ...
         VALUES
           ((SELECT id_categoria_persona FROM categoria_persona WHERE nombre = 'Proveedor'), $1, $2)
      ...
      for (const doc of dto.documentos) {
        ...
        const dtId = dt2[0].id_tipo_documento_persona;
        await client.query(
      ...
      return {
        ...
        message: 'Persona creada y vinculada como proveedor correctamente.'
      };
    } catch (err) {
